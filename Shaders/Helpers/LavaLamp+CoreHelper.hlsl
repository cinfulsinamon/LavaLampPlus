...
float3 GetLavaLighting(float3 viewDirection, LavaSurfaceParameters surfaceParameters, LavaLampMaterialParameters materialParameters)
{
    ...
    float3 modCoreColor = ModifyViaHSV(materialParameters.coreColor, _LavaHue, _LavaSaturation, _LavaValue);
	modCoreColor = ModifyViaHSV(modCoreColor, materialParameters.hue, materialParameters.sat, materialParameters.val);
	float3 modEdgeColor = ModifyViaHSV(materialParameters.edgeColor, _LavaHue, _LavaSaturation, _LavaValue);
	modEdgeColor = ModifyViaHSV(modEdgeColor, materialParameters.hue, materialParameters.sat, materialParameters.val);
	float3 scatteringColor = lerp(modCoreColor, modEdgeColor, thicknessFactor);
...
{ \
	...
	materialParametersStruct.hue = _LavaHue##id; \
	materialParametersStruct.sat = _LavaSaturation##id; \
	materialParametersStruct.val = _LavaValue##id; \
